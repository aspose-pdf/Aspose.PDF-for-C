#pragma once
/* Copyright (c) 2001-2016 Aspose Pty Ltd. All Rights Reserved. */

#include <system/shared_ptr.h>

#include "Generator/Drawing/PatternColorSpace.h"
#include "aspose_pdf_api_defs.h"

namespace Aspose { namespace Pdf { class Point; } }
namespace Aspose { namespace Pdf { class Color; } }

namespace Aspose {

namespace Pdf {

namespace Drawing {

/// <summary>
/// Represents gradient radial shading type.
/// </summary>
class ASPOSE_PDF_SHARED_API GradientRadialShading : public Aspose::Pdf::Drawing::PatternColorSpace
{
    typedef GradientRadialShading ThisType;
    typedef Aspose::Pdf::Drawing::PatternColorSpace BaseType;
    
    typedef ::System::BaseTypesInfo<BaseType> ThisTypeBaseTypesInfo;
    RTTI_INFO_DECL();
    
public:

    /// <summary>
    /// Gets starting circle center point.
    /// </summary>
    System::SharedPtr<Point> get_Start();
    /// <summary>
    /// Sets starting circle center point.
    /// </summary>
    void set_Start(System::SharedPtr<Point> value);
    /// <summary>
    /// Gets ending circle center point.
    /// </summary>
    System::SharedPtr<Point> get_End();
    /// <summary>
    /// Sets ending circle center point.
    /// </summary>
    void set_End(System::SharedPtr<Point> value);
    /// <summary>
    /// Gets starting circle radius.
    /// </summary>
    double get_StartingRadius();
    /// <summary>
    /// Sets starting circle radius.
    /// </summary>
    void set_StartingRadius(double value);
    /// <summary>
    /// Gets ending circle radius.
    /// </summary>
    double get_EndingRadius();
    /// <summary>
    /// Sets ending circle radius.
    /// </summary>
    void set_EndingRadius(double value);
    /// <summary>
    /// Gets start color.
    /// </summary>
    System::SharedPtr<Color> get_StartColor();
    /// <summary>
    /// Sets start color.
    /// </summary>
    void set_StartColor(System::SharedPtr<Color> value);
    /// <summary>
    /// Gets end color.
    /// </summary>
    System::SharedPtr<Color> get_EndColor();
    /// <summary>
    /// Sets end color.
    /// </summary>
    void set_EndColor(System::SharedPtr<Color> value);
    
    /// <summary>
    /// Initializes a new instance of the <see cref="GradientRadialShading"/> class.
    /// </summary>
    GradientRadialShading();
    /// <summary>
    /// Initializes a new instance of the <see cref="GradientRadialShading"/> class.
    /// </summary>
    /// <param name="startColor">The starting circle color.</param>
    /// <param name="endColor">The ending circle color.</param>
    GradientRadialShading(System::SharedPtr<Color> startColor, System::SharedPtr<Color> endColor);
    
protected:

    bool get_ExtendBeyondStart();
    void set_ExtendBeyondStart(bool value);
    bool get_ExtendBeyondEnd();
    void set_ExtendBeyondEnd(bool value);
    
    System::Object::shared_members_type GetSharedMembers() override;
    
private:

    System::SharedPtr<Point> start;
    System::SharedPtr<Point> end;
    System::SharedPtr<Color> startColor;
    System::SharedPtr<Color> endColor;
    double pr_StartingRadius;
    double pr_EndingRadius;
    bool extendBeyondStart;
    bool extendBeyondEnd;
    
};

} // namespace Drawing
} // namespace Pdf
} // namespace Aspose


